...
def starts_in_range(string, char_range):    
    #start is first char, end is char after- (could use any separator, - makes sense)
    start, end = char_range.split('-')
    #define as a range of characters and return what's in the range
    ord_char_range = range(ord(start), ord(end) + 1)
    return ord(string[0]) in ord_char_range

print("Starts between C and G, inclusive:")
c = [h.name for h in humans if starts_in_range(h.name, "C-G")]
print(c)
